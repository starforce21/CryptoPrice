...
function fetchMultipleTimes() {
  ...
  for(let i = 0; i < myPick.length; i++) {
    let response = fetch(`https://api.coingecko.com/api/v3/coins/${myPick[i]}`);
  ...
  .then(data=>{
    ...
    for (let i = 0; i < myPick.length; i++) {
        tableText+=`<tr><td>${data[i].market_cap_rank}</td>
                    <td><img style="width:18px; height: 18px;" src="${data[i].image.thumb}">${data[i].name}</td>
                    <td>${data[i].symbol}</td>
                    <td ${color(data[i].market_data.price_change_percentage_7d_in_currency.usd)}>${numberWithCommas(data[i].market_data.current_price.usd)}</td>
                    <td ${color(data[i].market_data.price_change_percentage_7d_in_currency.usd)}>${Math.round(data[i].market_data.price_change_percentage_7d_in_currency.usd*10)/10}%</td>
                    <td>${numberWithCommas(data[i].market_data.total_volume.usd)}</td>
                    <td>${numberWithCommas(data[i].market_data.market_cap.usd)}</td></tr>`
    }
    tableText+='</table>'
